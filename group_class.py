class Group:
    def __init__(self,name, text, skills=None,in_active=True):
        self.name = name
        self.text = text
        self.required_skills = skills if skills is not None else []
        self.in_active = in_active

    def add_skill(self,skill):
        '''Добавляет в список навыков новый навык'''
        if skill is not self.required_skills:
    ...
    def show_project(self):
        ...
        return (f'Проект: {self.name}\n'
                ...
                f'Список навыков/специалистов необходимых для проекта {self.required_skills}')
